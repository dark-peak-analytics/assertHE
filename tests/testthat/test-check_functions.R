...
test_that("find_test can identify a test where it exists", {
  ...
  path_to_test1 <- assertHE:::find_test(v_functions = "check_markov_trace",
                                        path = getwd(),
                                        test_path = "tests/testthat")

  testthat::expect_length(object = path_to_test1, n = 1)

  path_to_test2 <- assertHE:::find_test(v_functions = c("check_trans_prob_array", "mean"),
                                        path = getwd(),
                                        test_path = "tests/testthat")
...
test_that("tabulate_functions_in_folder_with_tests can identify functions, packages and test locations", {
  ...
  testthat::expect_silent(
    df_tests <<- assertHE:::tabulate_functions_in_folder_with_tests(
      path = getwd(),
  ...
  testthat::expect_false(object = {"base" %in% df_tests$package})
  testthat::expect_true(object = {"base" %in% df_tests2$package})
